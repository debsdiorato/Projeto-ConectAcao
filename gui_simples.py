...
# Imports dos seus módulos locais (assumindo que os arquivos existem na mesma pasta)
...
class App(tk.Tk):
    ...
    def build_vol_tab(self):
        ...
        ttk.Button(f, text="Atualizar lista", command=self.atualizar_vol).pack(padx=8, pady=6)

    def atualizar_vol(self):
        self.vol_listbox.delete(0, tk.END)
        for v in listar_voluntarios():
            self.vol_listbox.insert(tk.END, f"{v['id']} - {v['nome']} | skills: {v.get('habilidades','')}")
    ...
    def build_dem_tab(self):
        ...
        ttk.Button(btnf, text="Atualizar", command=self.atualizar_dem).grid(row=0, column=0, padx=4)
        ttk.Button(btnf, text="Criar demanda", command=self.criar_demanda_dialog).grid(row=0, column=1, padx=4)

    def atualizar_dem(self):
        self.dem_listbox.delete(0, tk.END)
        for d in listar_demandas():
            self.dem_listbox.insert(tk.END, f"{d['id']} - {d['titulo']} | skills: {d.get('habilidades_requeridas','')}")
